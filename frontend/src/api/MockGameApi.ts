...
export class MockGameApi implements GameApi {
  ...
  private create(state: SavedGame, bet: BetOption, theme: Theme, scenario: DemoScenario | null): Round {
    ...
    const maxMultiplier = theme === 'RED' ? 25 : 10;
    ...
    const crashMultiplier = scenario === 'win' ? 3.2 : scenario === 'crash' ? 1.3 : scenario === 'booster' ? 2.8
      : money(Math.min(maxMultiplier, Math.max(1.01, 0.96 / Math.max(0.000001, 1 - this.random()))));
    const round: Round = {
      id: crypto.randomUUID(), bet: { ...bet }, theme, levels, maxMultiplier, booster, boosterLevel,
  ...
  subscribeToRound(roundId: string, onRound: (round: Round) => void, onError: (error: Error) => void) {
    ...
    const tick = async () => {
      try {
        ...
        if (!stopped && ['active', 'cashed_out'].includes(round.status)) timer = setTimeout(tick, 50);
      } catch (cause) {
