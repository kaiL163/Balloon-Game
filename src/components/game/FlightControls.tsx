...
export function FlightControls({ round, pending, onCashout }: { round: Round; pending: boolean; onCashout: () => void }) {
  ...
  return <aside className="game-controls">
    ...
    <div className={`booster-status booster-${round.boosterState.toLowerCase()}`}>
      <BoosterChest state={round.boosterState} multiplier={round.booster} /><div>{round.booster === 1 ? 'Без сундука' : boosterLabels[round.boosterState]}<small>{round.boosterState === 'ACTIVATED' ? `Бустер x${round.booster} · +${20 * round.booster} очков` : round.boosterLevel ? `Уровень ${round.boosterLevel} · бустер без множителя, ×2, ×3 или ×4` : 'Спокойный полёт'}</small></div>
    </div>
