...
export function useFlight(scenario: DemoScenario | null) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      if (!cancelled && current.current && !busy.current) {
        busy.current = true;
        try {
          const next = await gameApi.advanceRound(current.current.id);
          if (!cancelled) { current.current = next; setRound(next); }
        } catch (cause) {
          if (!cancelled) setError(cause instanceof Error ? cause.message : 'Не удалось обновить полёт.');
        } finally { busy.current = false; }
      }
      if (!cancelled) timer = setTimeout(tick, 50);
    }
